...
assert.equal(review.scanAreas.find((area) => area.id === 'business_information')?.state, 'issue')
assert.equal(review.scanAreas.find((area) => area.id === 'ai_discovery')?.state, 'issue')
