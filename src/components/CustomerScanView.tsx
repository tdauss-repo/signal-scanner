...
export function CustomerScanView(props: Props) {
  ...
  return <div className="customer-shell">
    ...
    <nav className="customer-nav operator-primary-nav" aria-label="Primary operator workflow">{views.map((name, index) => <button key={name} type="button" aria-current={view === name ? 'page' : undefined} onClick={() => setView(name)}><span className="workflow-step-number">{index + 1}</span>{name}{name === 'Review' && readiness.awaitingDisposition > 0 ? <span className="workflow-step-count">{readiness.awaitingDisposition}</span> : null}</button>)}</nav>
    <main id="customer-content" className="customer-content">
      ...
      {view === 'Business' ? <BusinessWorkspacePanel state={state} currentScanId={props.currentScanId} dirty={props.dirty} scans={props.scans} onProfileChange={props.onProfileChange} onSaveCurrent={props.onSaveCurrent} onSaveAsNew={props.onSaveAsNew} onLoad={props.onLoad} onDuplicate={props.onDuplicate} onRename={props.onRename} onDelete={props.onDelete} onExport={props.onExportFullScan} onImport={props.onImportFullScan} onStartBlank={props.onStartBlank} onGoScan={() => setView('Scan')} /> : null}
      {view === 'Scan' ? <>
        <section className="operator-scan-grid" aria-label="Scan and evidence status"><article className="operator-scan-card"><p className="customer-eyebrow">Visibility scan</p><h3>{loading ? 'Scan in progress' : summary.snapshot.overall}</h3><p>Check how this business appears across Website &amp; Technical, Search &amp; Maps, Business Information, and AI Discovery.</p><p>{summary.snapshot.detail}</p><p className="customer-small">Last activity: {lastActivity ? new Date(lastActivity).toLocaleString() : 'Not scanned'}</p><button type="button" className="customer-primary" disabled={loading || !completeness.readyToScan} onClick={props.onScan}>{loading ? 'Running visibility scan…' : lastRun ? 'Re-run Visibility Scan' : 'Run Visibility Scan'}</button>{!completeness.readyToScan ? <p className="customer-small">Business name and website are required before scanning.</p> : null}</article>
        ...
        <section className="customer-next"><div><p className="customer-eyebrow">{lastRun && !loading ? 'Scan complete' : 'Next step'}</p><h3>{primaryCandidates.length} potential {primaryCandidates.length === 1 ? 'finding requires' : 'findings require'} operator review</h3><p>Review captured evidence, refine customer wording where needed, and approve or dismiss each primary candidate.</p></div><button className="customer-primary" type="button" onClick={() => setView('Review')}>Continue to Review →</button></section>
      </> : null}
