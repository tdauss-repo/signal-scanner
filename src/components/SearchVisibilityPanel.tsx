...
import { scanStateLabel } from '../utils/visibilityScanState'
...
export function SearchVisibilityPanel({ profile, profileState, legacyTests, observations, onChange, onAddToActionPlan }: Props) {
  ...
  return <section className="panel search-visibility-panel">
    ...
    <div className="search-query-list">{queries.map((query) => <article className="search-query-card" key={query.id}>
      ...
      {searchDestinations.map((destination) => { const value = { ...defaultSearchDestinationObservation(destination, query.query), ...observations[query.id]?.[destination] }; const types = resultTypesForDestination(destination); const assistedCurrent = operatorAssistedEvidenceIsCurrent(value, profile, profileState); const assistedGoogleBrand = query.id === 'search-brand-canonical' && destination === 'Google Search' && value.automation?.state === 'interactive_review_required' && ['access_blocked', 'acquisition_failure', 'runtime_unavailable'].includes(value.automation.assessment?.blocker || ''); return <details className="destination-observation" key={destination}>
        <summary>{destination} — {assistedCurrent ? searchVisibilityResultLabel(value.overallResult) : value.automation && !value.reviewed ? scanStateLabel[value.automation.state] : searchVisibilityResultLabel(value.overallResult)}</summary>
        <div className="directory-actions"><a href={value.automation?.inspectedUrl || publicPresenceUrl(destination, query.query)} target="_blank" rel="noreferrer">Open {destination}</a></div>
        {value.automation ? <details><summary>{value.automation.automaticObservation ? 'Automatically matched observation — available for inspection' : 'Automated evidence — review required'}</summary><p>{value.automation.evidence || 'No reliable interpretation. Use the public destination link for interactive review.'}</p>{value.automation.assessment ? <><p>Identity confidence: {value.automation.assessment.confidence} · Query mode: {value.automation.queryMode}</p><p>{value.automation.assessment.ambiguityReasons.join(' ')}</p><p>Tier-3 candidate: {value.automation.assessment.tier3Candidate ? 'Yes — public interaction observed; no login or challenge bypass' : 'No established interaction blocker'}</p><pre>{JSON.stringify(value.automation.assessment.matches, null, 2)}</pre></> : null}<pre>{JSON.stringify(value.automation.captures, null, 2)}</pre></details> : null}
